...
class InventoryManager:
    ...
    def notify(self, event):
        if event.etype == 'weight_change':
            ...
            if slot_num >= self._slots:
                print("Slot", slot_num, "does not exist")
                messagebox.showerror(
                    "Error", message="Slot "+str(slot_num)+" does not exist")
                return
            med_name = self._medicine_at[slot_num]
            if med_name == '':
                messagebox.showerror(
                    "Error", message="Slot "+str(slot_num)+" has no medicine")
                return
            med_weight = self._medicines[med_name]['weight']
            diff = round(event.data['weight'] / med_weight) - \
                self._inventory._slot[slot_num]
            self._inventory._slot[slot_num] = round(
                event.data['weight'] / med_weight)
            self._event_queue.new_event(Event('pill_change', {
                                        med_name: diff, 'medicine': med_name, 'time': event.data['time']}))
