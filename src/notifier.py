...
class Notifier:
    ...
    def __init__(self, event_queue, email, password):
        ...
        config = {
            ...
            "serviceAccount": "./access_key/intelligent-medicine-box-3f8a9b863bd1.json"
        }
        ...
        if email is None or password is None:
            ...
            assert(False)
        self._user = auth.sign_in_with_email_and_password(email, password)  # login the user.
        #print(user)
    ...
    def _send_alert(self, alert_event):
        ...
        self._db.child('/patients/' + self._patient_id + '/notifications/').push(alert_event.data, self._user['idToken'])
        
    def _send_pres(self, presc):
        print('sending prescription', presc)
        self._db.child('/patients/' + self._patient_id + '/prescriptions/').child('pres'+presc['id']).set(presc, self._user['idToken'])
